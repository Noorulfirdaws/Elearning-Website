...
import {
  ...
  HelpCircle, ExternalLink, ChevronDown, ChevronUp, Youtube, Trophy
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex gap-1 mt-3 overflow-x-auto pb-1">
            {TABS.map(tab => {
              ...
              const hasContent = tab === 'cours' ? true
              ...
              return (
                ...
                >
                  ...
                  {tab === 'quiz' && quiz.length > 0 && (
                    <span className="ml-1 text-xs bg-white/20 px-1.5 py-0.5 rounded-full">{quiz.length}</span>
                  )}
                </button>
              );
            })}
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <AnimatePresence mode="wait">
          ...
          {/* ─── COURS ─────────────────────────────────────────────── */}
          {activeTab === 'cours' && (
            <motion.div key="cours" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              {/* Introduction */}
              {contenu?.introduction && (
                <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-2xl p-5 mb-6">
                  <p className="text-blue-900 dark:text-blue-100 leading-relaxed">{contenu.introduction}</p>
                </div>
              )}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">{point.titre}</h3>
                        <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{point.explication}</p>
                        {point.formule && (
              ...
              {chapitre.youtubeUrl && (
                <div className="mt-8 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-2xl p-5">
                  ...
                  >
                    <ExternalLink className="h-4 w-4" /> Voir sur YouTube
                  </a>
                </div>
              )}
          ...
          {/* ─── EXEMPLES ──────────────────────────────────────────── */}
          {activeTab === 'exemples' && (
            <motion.div key="exemples" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
              {exemples.map((ex: any, i: number) => (
                <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
                  <div className="bg-amber-50 dark:bg-amber-950 border-b border-amber-100 dark:border-amber-900 px-5 py-3">
                    <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wide mb-1">Exemple {i + 1}</p>
                    <p className="text-gray-800 dark:text-gray-200 font-medium">{ex.situation}</p>
                  </div>
                  <div className="px-5 py-4">
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Résolution pas à pas</p>
              ...
              <div className="flex justify-end">
                {exercices.length > 0 && (
                  <button onClick={() => setActiveTab('exercices')} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors">
          ...
          {/* ─── EXERCICES ─────────────────────────────────────────── */}
          {activeTab === 'exercices' && (
            <motion.div key="exercices" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
              {exercices.map((ex: any, i: number) => (
                <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
                  <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                    <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-1">Exercice {i + 1}</p>
                    <p className="text-gray-900 dark:text-white font-medium leading-relaxed">{ex.enonce}</p>
                  </div>
                  <div className="px-5 py-3">
                    ...
                    <AnimatePresence>
                      {corrigesVus.has(i) && (
                        ...
                        >
                          <p className="text-xs font-semibold text-green-700 dark:text-green-400 mb-2">✅ Corrigé détaillé</p>
              ...
              <div className="flex justify-end">
                {quiz.length > 0 && (
                  <button onClick={() => setActiveTab('quiz')} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors">
                    Faire le quiz →
                  </button>
                )}
              </div>
            ...
            <motion.div key="quiz" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              {!quizStarted && !quizDone ? (
                <div className="text-center py-12">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">Quiz de validation</h2>
                  <p className="text-gray-500 mb-2">{quiz.length} question{quiz.length > 1 ? 's' : ''} sur <strong>{chapitre.titre}</strong></p>
                ...
                <div className="space-y-6">
                  {quiz.map((q: any, i: number) => (
                    <QuizQuestion
                      ...
                      onChoisir={(opt) => choisirReponse(i, opt)}
                    />
                  ))}
                  <button
                    onClick={validerQuiz}
...
function QuizQuestion({ question, index, repChoisie, onChoisir }: any) {
  return (
...
function QuizResultats({ score, quiz, reponses, onRecommencer }: any) {
  ...
  return (
    <div>
      ...
      <div className={cn(
        ...
        reussi ? 'bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800'
               : 'bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800'
      )}>
        <div className="text-6xl mb-3">{reussi ? '🏆' : '📚'}</div>
        <div className={cn('text-5xl font-bold mb-2', reussi ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400')}>
          {score}%
        </div>
        <p className={cn('text-lg font-semibold mb-1', reussi ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300')}>
          {reussi ? 'Bravo ! Chapitre validé ✅' : 'Continue à réviser 💪'}
        </p>
        <p className="text-sm text-gray-500">
          {quiz.filter((_: any, i: number) => reponses[i] === quiz[i].reponse_correcte).length} bonne{quiz.filter((_: any, i: number) => reponses[i] === quiz[i].reponse_correcte).length > 1 ? 's' : ''} réponse{quiz.filter((_: any, i: number) => reponses[i] === quiz[i].reponse_correcte).length > 1 ? 's' : ''} sur {quiz.length}
        </p>
      </div>
        ...
        {quiz.map((q: any, i: number) => {
          ...
          return (
            <div key={i} className={cn(
              ...
              correct ? 'bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800'
                      : 'bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800'
            )}>
              <div className="flex items-start gap-3 mb-2">
                {correct ? <CheckCircle className="h-5 w-5 text-green-600 flex-shrink-0 mt-0.5" /> : <XCircle className="h-5 w-5 text-red-600 flex-shrink-0 mt-0.5" />}
              ...
              {!correct && (
                <p className="text-sm text-gray-600 dark:text-gray-400 ml-8 mb-1">
                  Ta réponse : <span className="text-red-600 font-medium">{reponses[i]}</span> |
                  Bonne réponse : <span className="text-green-700 dark:text-green-400 font-medium"> {q.reponse_correcte}</span>
                </p>
              )}
